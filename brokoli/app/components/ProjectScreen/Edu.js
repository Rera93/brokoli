...
export default class Edu extends React.Component{
    ...
    componentDidMount(){
        ...
        this.state.eduData = this.props.navigation.state.params.selectedEduExp
        this.setState(function(prevState, props){
             return { eduData: prevState.eduData }
        })
        console.log('Passed eduData: ', this.state.eduData)
    }
        ...
        return(
            ...
            renderItem={({ item, index }) => (
              <View style={styles.item}>
                ...
                {/*Name of project*/}
                <Image style={styles.icon} source={require('../../../img/icons/college-graduation.png')} />
                <Text style={styles.name}> {item.school} </Text>
                </View>
                <View style={styles.reactions}>
                {/*Number of applicants until now*/}
                <Image style={styles.icon} source={require('../../../img/icons/graduate-certificate.png')} />
                <Text style={{fontSize: 17, fontWeight: '400', color: '#C7C7CD'}}> {item.degree}  </Text>
                </View>
                <View style={styles.reactions}>
                {/*Number of available positions*/}
                <Image style={[styles.icon, {tintColor: '#42D260' }]} source={require('../../../img/icons/medal.png')} />
                <Text style={{fontSize: 17, fontWeight: '400', color: '#C7C7CD'}}> {item.spec}  </Text>
                </View>
                <View style={styles.reactions}>
